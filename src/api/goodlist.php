<?php
    include 'connect.php';
    $APItype=isset($_POST['APItype']) ? $_POST['APItype'] : 'goodlist';
    //查找相应id在详情页输出
    ...
    if($APItype=='goodlist'){
        ...
        $sqlall="SELECT * FROM goodlist";
        $resall=$conn->query($sqlall); //得到结果集
        $rowall=$resall->fetch_all(MYSQLI_ASSOC);
        $resall->close();//关闭结果集   
    }
    //列表页数据降序输出
    ...
    }else if($APItype=='goodlist'||$APItype=='goodlistdown'||$APItype=='goodlistup'){
        $datalist=array(
            'listall'=>$rowall,
            'list'=>$row,
            'page'=>$page,
